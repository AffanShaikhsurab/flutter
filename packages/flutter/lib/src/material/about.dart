...
class _LicensePageState extends State<LicensePage> {
  ...
  Widget _packagesView(final BuildContext _, final bool isLateral) {
    ...
    return _PackagesView(
        about: about, isLateral: isLateral, selectedId: selectedId);
  }
}

class _AboutProgram extends StatelessWidget {
  const _AboutProgram(
      {required this.name, required this.version, this.icon, this.legalese});
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.symmetric(
          horizontal: _getGutterSize(context), vertical: 24.0),
      child: Column(
        children: <Widget>[
          Text(name,
...
class _PackagesView extends StatefulWidget {
  const _PackagesView(
      {required this.about, required this.isLateral, required this.selectedId});
